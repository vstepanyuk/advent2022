...
class Solution {
  ...
  check(left, right) {
    ...
    if (Array.isArray(left) && Array.isArray(right)) {
      for (let i = 0; i < Math.max(left.length, right.length); i++) {
        ...
        let result = this.check(left[i], right[i]);
        if (result === undefined) {
          continue;
        }
  ...
  part1() {
    const results = [];
    for (let [left, right] of this.input) {
      const result = this.check(left, right);
      results.push(result);
    }

    let sum = results.reduce((prev, result, index) => {
      return prev + (result ? index + 1 : 0);
    }, 0);
  ...
  part2() {
    let part2 = this.input
      .filter(Boolean)
      .flat(1)
      .concat([[[2]], [[6]]])
      .sort((a, b) => {
        let r = this.check(a, b);
        if (r == undefined) {
          return 0;
        } else if (r) {
          return -1;
    ...
    let index1 = part2.findIndex((a) => JSON.stringify(a) === "[[6]]") + 1;
    let index2 = part2.findIndex((a) => JSON.stringify(a) === "[[2]]") + 1;
